...
class AlphiMail
{
	...
	/**
	 * Add carbon copy invisible email address
	 *
	 * @param string $address
	 * @param string|null $name
	 * @return Email
	 */
	public function addCci($address, $name = null)
	{
		$this->cci[] = array($address, $name);
	...
	{
		//On met ça la car il y a un warning mais aucune idée
		$message ="";
		//Mais ce fix fonctionne x)
		...
		if (!empty($this->cci)) {
			$this->headers['Cci'] = $this->formatAddressList($this->cci);
		}
